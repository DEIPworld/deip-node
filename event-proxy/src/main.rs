...
type RuntimeT = node_template_runtime::Runtime;

use app::{
    Actor, ActorI, ActorIO, ActorO, BlockchainActor, BlockchainActorIO, BlockchainActorInput,
    BlockchainActorOutput, BlockchainActorOutputData, BlocksReplay, MessageBrokerActor,
    MessageBrokerActorIO, MessageBrokerActorInput, MessageBrokerActorOutput,
    MessageBrokerActorOutputData, OffchainActor, OffchainActorIO, OffchainActorInput,
    OffchainActorOutput, OffchainActorOutputData,
};
...
async fn main() {
    ...
    let config_file = args.value_of_t_or_exit("config");
    ...
    loop {
        tokio::select! {
            ...
            Some(SubscriptionBufferTaskResult {
                ...
                let finalized_block_header: RuntimeT::Header = subscription_item;
...
async fn blocks_replay_task(replay: BlocksReplay) -> (BlocksReplay, Option<RuntimeT::Header>) {
