...
use app::{
    ...
    FinalizedBlocksSubscriptionItem, LastKnownBlock, MaybeBlockEvent, MessageBrokerActor,
    MessageBrokerActorIO, MessageBrokerActorInput, MessageBrokerActorOutput,
    MessageBrokerActorOutputData, MessageBrokerConfigureResult, OffchainActor, OffchainActorIO,
...
macro_rules! reset {
    ($actor_task_queue:ident, $_released_actor_queue:ident, $offchain_config:expr) => {
        $actor_task_queue.push(
            init_actor_task::<_, _, OffchainActorIO>(
...
macro_rules! reset_blockchain_actor {
    ($actor_task_queue:ident, $_released_actor_queue:ident, $blockchain_config:expr) => {
        $actor_task_queue.push(
            init_actor_task::<_, _, BlockchainActorIO>(
...
async fn main() {
    ...
    let config =
        config::load::<OffchainConfig<LastKnownBlock>, _>(config_file).unwrap_or_else(|e| {
            error!("{}\n EXIT(-1)", e.to_string());
            exit(-1);
        });
    ...
    info!("message broker actor spawned");
    ...
    let mut released_message_broker_actor_queue =
        released_actor_queue::<_, _, MessageBrokerActorIO>();
    ...
    loop {
        ...
        Some(SubscriptionBufferTaskResult {
            ...
            let finalized_block_header: <RuntimeT as Config>::Header = subscription_item;
                        ...
                        Ok(subscription) => {
                            ...
                            match maybe_head_block {
                                Some(Ok(head_block)) => {
                                    ...
                                    let task = init_actor_task::<_, _, BlockchainActorIO>(
                                        replay_blocks,
                                        &mut released_blockchain_actor_queue
                                    ).await;
                                    blockchain_actor_task_queue.push(task);
        ...
        Some(EventsBufferTaskResult {
            ...
            let input = MessageBrokerActorInput::send_block_event(event, events_buffer, remaining, subscription_buffer);
            let task = init_actor_task::<_, _, MessageBrokerActorIO>(input, &mut released_message_broker_actor_queue).await;
        ...
        Some(message_broker_actor_task_result) = message_broker_actor_task_queue.next() => {
            ...
            let delivery_status = match output {
                ...
                MessageBrokerActorOutput::Result(MessageBrokerActorOutputData::Configure(MessageBrokerConfigureResult { maybe_error, ctx })) => {
                    match maybe_error {
                ...
                MessageBrokerActorOutput::Result(MessageBrokerActorOutputData::SendReplayedBlockEvent(SendEventResult { delivery, ctx })) => {
                    if ctx.remaining > 0 {
                        // println!("replayed_block_events_buffer_task AGAIN: remaining={:?}", ctx.remaining);
                        let task = replayed_block_events_buffer_task(ctx.remaining, ctx.replay);
                        replayed_block_events_buffer_task_queue.push(task);
                    } else {
                ...
                MessageBrokerActorOutput::Result(MessageBrokerActorOutputData::SendBlockEvent(SendEventResult { delivery, ctx })) => {
                    if ctx.remaining > 0 {
                        // println!("events_buffer_task_queue AGAIN: remaining={:?}", ctx.remaining);
                        let task = events_buffer_task(ctx.events_buffer, ctx.remaining, ctx.subscription_buffer);
                        events_buffer_task_queue.push(task);
                    } else {
                        // Process the next finalized block:
                        // println!("SEND NEXT SUBSCRIPTION TASK");
                        let task = subscription_buffer_task(ctx.subscription_buffer, ctx.events_buffer);
                        subscription_buffer_task_queue.push(task);
                    }
                    delivery
                },
            };
...
impl<T> Buffer<T> {
    ...
    async fn pop(&mut self) -> T {
        if let Some(item) = self.1.recv().await {
            item
        } else {
            panic!("NEVER GONE");
        }
...
fn released_actor_queue<I, O, IO: ActorIO<I, O>>() -> ReleasedActorQueue<IO::Pair> {
    mpsc::channel(1)
}
...
async fn subscription_task(
    mut subscription: FinalizedBlocksSubscription,
    buf: SubscriptionBufferIn,
...
async fn events_buffer_task(
    mut events_buffer: EventsBuffer,
    remaining: usize,
    subscription_buffer: SubscriptionBuffer,
...
async fn replayed_block_events_buffer_task(
    remaining: usize,
    mut replay: BlocksReplay,
) -> ReplayedBlockEventsBufferTaskResult {
    if remaining == 0 {
        panic!("UNEXPECTED");
    }
    let event = replay.3.pop().await;
    let remaining = remaining - 1;
    ReplayedBlockEventsBufferTaskResult { event, remaining, replay }
}

async fn blocks_replay_task(
    replay: BlocksReplay,
) -> (BlocksReplay, Option<<RuntimeT as Config>::Header>) {
