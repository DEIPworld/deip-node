...
pub type ListingOf<T> =
    Listing<<T as SystemConfig>::AccountId, BalanceOf<T>, <T as SystemConfig>::BlockNumber>;

pub type OfferOf<T> =
    Offer<<T as SystemConfig>::AccountId, BalanceOf<T>, <T as SystemConfig>::BlockNumber>;
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        type Tokens: NFTImplT<Fingerprint = Self::Token, Account = Self::AccountId>;
    ...
    pub enum Event<T: Config> {
        ...
        PriceChanged { owner: T::AccountId, token: T::Token, price: Option<BalanceOf<T>> },
        /// Token was sold to a new owner
        Sold { owner: T::AccountId, buyer: T::AccountId, token: T::Token, price: BalanceOf<T> },
        /// Token listed on Marketplace
        Listed { owner: T::AccountId, token: T::Token, price: BalanceOf<T> },
        /// Token unlisted on Marketplace
        Unlisted { owner: T::AccountId, token: T::Token },
        /// Offer was placed on a token
        OfferAdded { offerer: T::AccountId, token: T::Token, price: BalanceOf<T> },
        /// Offer was withdrawn
        OfferWithdrawn { offerer: T::AccountId, token: T::Token },
        /// Offer was accepted
        OfferAccepted { owner: T::AccountId, offerer: T::AccountId, token: T::Token },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[transactional]
        pub fn unlist(origin: OriginFor<T>, token: T::Token) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        #[transactional]
        pub fn buy(origin: OriginFor<T>, token: T::Token, value: BalanceOf<T>) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        #[transactional]
        pub fn withdraw_offer(origin: OriginFor<T>, token: T::Token) -> DispatchResult {
            let who = ensure_signed(origin)?;
...
impl<T: Config> Pallet<T> {
    ...
    fn change_owner(
        ...
        transfer_item::<T::Tokens>(token.clone(), from, to).map_err(|_| Error::<T>::NonTransferable)
    }

    fn owner(token: &T::Token) -> Option<T::AccountId> {
        pick_item::<T::Tokens>(token.clone()).ok().map(|x| x.account().clone())
    }
