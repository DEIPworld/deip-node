...
use frame_support::{
    dispatch::DispatchResultWithPostInfo, pallet_prelude::*, weights::GetDispatchInfo, Hashable,
};

use deip_transaction_ctx::{TransactionCtxId, TransactionCtxT};

use crate::storage::{StorageOps, StorageOpsT};

use super::{Config, Error, Event, ProposalRepository, *};
...
#[allow(type_alias_bounds)]
pub type ProposalBatchItemOf<T: Config> =
    BatchItem<<T as frame_system::Config>::AccountId, <T as Config>::Call>;

#[allow(type_alias_bounds)]
pub type InputProposalBatchItem<T: Config> = BatchItem<T::DeipAccountId, <T as Config>::Call>;
...
impl<T: Config> DeipProposal<T> {
    ...
    pub fn create(
        ...
        ensure!(!ProposalRepository::<T>::contains_key(&id), Error::<T>::AlreadyExist);
        match crate::batch_assertions::assert_proposal::<T, _>(
            &batch,
            &id,
            BATCH_MAX_DEPTH,
            BATCH_MAX_SIZE,
        ...
        let batch: ProposalBatch<T> = batch
            ...
            .map(|x| {
                ...
                BatchItem { account: account.into(), call }
            })
            .collect();

        use sp_std::iter::FromIterator;
        let decisions = BTreeMap::from_iter(
            batch.iter().map(|x| (x.account.clone(), ProposalMemberDecision::Pending)),
        );
        ...
        match member_decision.decide(decision) {
            ...
            Ok(Some(ProposalMemberDecision::Approve)) =>
                if self.ready_to_exec() {
                    ...
                    storage_ops.push_op(StorageOps::DeleteProposal(self));
                    let weight =
                        WeightInfoOf::<T>::decide_final_approve().saturating_add(exec_weight);
    ...
    fn ready_to_exec(&self) -> bool {
        let approved = self
            .decisions
            .values()
            .all(|x: &ProposalMemberDecision| matches!(x, ProposalMemberDecision::Approve));
        approved && matches!(self.state, ProposalState::Pending)
    }

    pub(crate) fn expired(&self, now: T::Moment) -> bool {
        (self.created_at + T::Ttl::get()) <= now && matches!(self.state, ProposalState::Pending)
    }
